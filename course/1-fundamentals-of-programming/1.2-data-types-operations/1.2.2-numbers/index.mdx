...
Before we dive in, let's locate where all the number magic happens. In the Pybricks Code Editor, look for the green **Data** section in the left sidebar. Its the first subsection called **Numbers**:
...
In the green **Math** section, you'll find the arithmetic block - it's like having a mini calculator right in your code. This block lets you do all the basic math operations: addition (+), subtraction (-), multiplication (×), and division (÷).
...
When you divide in Python, it always gives you a float (decimal number) as the result, even if the answer could be a whole number. It's like the computer saying, "I did division, so I'm giving you the most precise answer possible!" This is actually helpful because division doesn't always result in whole numbers (think 10 ÷ 3 = 3.333...).

...
<Meta title="1. Fundamentals of Programming/1.2 Data Types & Operations/1.2.3 Booleans/1.2.3.3 Solutions" />

# 1.2.3.3 Booleans - Solutions
...
<img
  src="/1-fundamentals-of-programming/1.2-data-types-operations/1.2.3-booleans/res/SOL-1.2.png"
...
#### 8. Sensor range validation — solution and explanation
...
# The main program starts here.
print((sensor_reading < 10) or (sensor_reading > 50))
```

Explanation:

- We check if the sensor reading (45) is outside the valid range of 10 to 50.
- "Outside the range" means either less than 10 OR greater than 50.
- `45 < 10` is False, and `45 > 50` is False.
- False OR False equals False (the sensor is within range).
...
#### 12. Chained comparisons with variables — solution and explanation
...
- `min_val < sensor < max_val` → `5 < 12 < 20` → True (12 is between 5 and 20)
- `0 <= sensor <= 10` → `0 <= 12 <= 10` → False (12 is not between 0 and 10)
